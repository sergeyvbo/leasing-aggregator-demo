...
interface BrokerDocumentModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (document: BrokerDocument) => void;
}

const BrokerDocumentModal: React.FC<BrokerDocumentModalProps> = ({
  document,
  isOpen,
  onClose,
  onSave
}) => {
  const [formData, setFormData] = useState<Partial<BrokerDocument>>({
    type: 'other',
    title: '',
    fields: {},
    issueDate: '',
  ...
  const handleTypeChange = (newType: string) => {
    setFormData(prev => ({
      ...prev,
      type: newType as BrokerDocument['type'],
  ...
  const handleAddAttachment = () => {
    ...
    const mockAttachment: BrokerAttachment = {
      id: `att_${Date.now()}`,
  ...
  const handleViewAttachment = (attachment: BrokerAttachment) => {
  ...
  const handleSave = () => {
    ...
    const documentToSave: BrokerDocument = {
      id: document?.id || `doc_${Date.now()}`,
      type: formData.type as BrokerDocument['type'],
...
export default BrokerDocumentModal;
