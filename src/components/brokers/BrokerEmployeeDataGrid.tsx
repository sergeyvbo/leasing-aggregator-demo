...
import type { BrokerEmployee, BrokerEmployeeDataGridProps } from '../../types/brokers';
import { EmptyState } from '../common';
import BrokerEmployeeActionsColumn from './BrokerEmployeeActionsColumn';
...
const BrokerEmployeeDataGrid: React.FC<BrokerEmployeeDataGridProps> = ({ 
  ...
  // Summary configuration for broker employee statistics
  const summaryConfig: SummaryConfig<BrokerEmployee> = {
    calculate: (data: BrokerEmployee[]) => ({
  ...
  const columnsWithActions: ColumnDefinition<BrokerEmployee>[] = [
    ...columns,
    {
      key: 'actions',
      title: 'Действия',
      sortable: false,
      width: 'w-32',
      render: (_, employee: BrokerEmployee) => (
        <BrokerEmployeeActionsColumn
          employee={employee}
          onEdit={onEditEmployee}
          onDelete={onDeleteEmployee}
          onApprove={onApproveEmployee}
          onReject={onRejectEmployee}
        />
      ),
    },
  ];
  ...
  if (employees.length === 0) {
    return (
      <EmptyState
        ...
        description="У данного брокера пока нет зарегистрированных сотрудников."
      />
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <DataGrid
        ...
        className="broker-employee-grid min-w-full"
      />
    </div>
  );
};

export default BrokerEmployeeDataGrid;
