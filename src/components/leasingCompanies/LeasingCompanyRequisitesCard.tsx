...
import type { LeasingCompanyRequisitesCardProps } from '../../types/leasingCompanies';
...
const LeasingCompanyRequisitesCard: React.FC<LeasingCompanyRequisitesCardProps> = ({
  ...
  // Get fields to display for leasing company
  const getFieldsForLeasingCompany = () => {
    return [
      'fullName',
      'inn',
      'kpp',
  ...
  const fieldsToDisplay = getFieldsForLeasingCompany();
      ...
      <div className="px-4 md:px-6 py-4 border-b border-gray-200 bg-gray-50">
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            <h3 className="text-base md:text-lg font-semibold text-gray-900">Реквизиты лизинговой компании</h3>
          ...
          {fieldsToDisplay.map((fieldKey) => {
            ...
            return (
              <div key={fieldKey} className="space-y-1">
                ...
                {isEditing ? (
                  <input
                    type={fieldKey.includes('Date') ? 'date' : fieldKey === 'email' ? 'email' : 'text'}
                    value={value || ''}
...
export default LeasingCompanyRequisitesCard;
